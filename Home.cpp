...
int main(int, char **)
{
    ...
    while (not finish)
    {
		...
		if(keys[SDL_SCANCODE_RIGHT] and icon_x < WIDTH - 100){ icon_x += 3; f = true;}//тут и далее цифра 3 это скорость полета
		...
		if(icon_x2 < WIDTH - 150 and f2){
			...
			icon_x2 += 2;//движение вправо
		}
		else f2 = false;
		if(icon_x2 > 0 and !f2){
			icon2 = Make_icon(ren.get(), "ezhik_l.png");
			icon_x2 -= 2;//движение влево
		}
		else f2 = true;
